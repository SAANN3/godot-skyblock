...
func _ready() -> void:
	...
	var grass: BlockObject = _resource_system.get_object("grass")
	var leaf: BlockObject = _resource_system.get_object("leaf")
	var wood: BlockObject = _resource_system.get_object("wood")
		...
		for x: int in range(0, 3):
			for y: int in range(0, 3):
				for z: int in range(0, 3):
					grass.clone().place_block(Vector3(x, y, z) + offset, self)
	# Create a tree.
	# trunk.
	for y: int in range(0, 4):
		wood.clone().place_block(Vector3(2, y - 2, -3), self)
	# leafs
	for y: int in range(0, 4):
		var ring_size := 2 if y < 2 else 1
		for x: int in range(-ring_size, ring_size + 1):
			for z: int in range(-ring_size, ring_size + 1):
				leaf.clone().place_block(Vector3(2 + x, y + 2, -3 + z), self)
	pass
	# Spawn player.
